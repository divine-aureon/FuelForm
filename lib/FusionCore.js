...
export function calculateRecoveryFuel({
  ...
  //BASE NEAT//
  const awakeHours = 16;
  const baselineNEAT = 0.3 * weight_kg * awakeHours;

  //RECOVERY TDEE//
  const rawRecoveryTDEE = BMR + baselineNEAT;
...
export function calculateActiveFuel({
  ...
  //EAT//
  const calsPerMin = { none: 0, light: 5, moderate: 6.5, high: 9 }[exerciseIntensity] || 0;
  const EAT = exerciseMinutes * calsPerMin;

  //ACTIVE TDEE//
  const rawActiveTDEE = BMR + baselineNEAT + stepCals + EAT;
