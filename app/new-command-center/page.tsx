...
import PrimeTasksTile from "@/components/CoreTiles/PrimeTasks"
...
export default function PaidCommandCenter() {
  ...
  const [selectedPage, setSelectedPage] = useState<PageId>("1");
  ...
  const getNavigationTargets = (current: PageId): { left: PageId; right: PageId } => {
    if (current === "1") return { left: "3", right: "2" };
    if (current === "2") return { left: "1", right: "3" };
    return { left: "2", right: "1" };
  };

  const { left, right } = getNavigationTargets(selectedPage);
    ...
    <>
      <PageFadeWrapper>
        ...
        border-white/30 shadow-xl text-white text-2xl glowing-button">
          ...
          <div className="fixed bottom-28 left-0 w-full flex justify-between z-30">
            <div className="grid grid-cols-6 w-full mt-2">
              <button
                onClick={() => handlePageClick(left)}
                className={`flex justify-center pb-2 pl-2 pt-4 pr-4 w-full rounded-tr-[140px] rounded-tl-3xl rounded-bl-3xl rounded-br-3xl shadow glowing-home-button`}>
              ...
              <button
                ...
                className={`flex justify-center pb-2 pl-2 pt-4 pl-4 w-full rounded-tl-[140px] rounded-tr-3xl rounded-bl-3xl rounded-br-3xl shadow glowing-home-button`}>
