...
export default function RepSync() {
  ...
  const currentDropSetTarget = useGlobalData((s) => s.currentDropSetTarget);

  const setCurrentDropSetTarget = useGlobalData((s) => s.setCurrentDropSetTarget);
  ...
  const addSetToMovement = (movementName: string) => {
    setWorkoutSessionData((prev) => {
      ...
      setCurrentDropSetTarget(newSetKey);
      return {
        ...prev,
        [movementName]: {
          ...existing,
  ...
  //{ weight: "", reps: "", locked: false, dropset: false }
  //{ weight: "", reps: "", locked: false, dropset: true }

  //{set.dropset && (
  //<span className="ml-2 text-xs text-yellow-400 font-bold">Drop Set</span>
  //)}
    ...
    <>
      ...
      <div>
        ...
        border-white/30 shadow-xl text-white text-3xl glowing-button mb-2">
          <div className="absolute flex flex-col pb-2 items-center bg-indigo-500/30 justify-center inset-0 text-center rounded-xl">
            <div className="flex items-center gap-2">RepSync</div>
      ...
      {activeSessionStatus ? (
        <>
          ...
          <div className="bg-white/30 rounded-xl p-4 mb-2">
            ...
            {TodaysWorkout.map((movementName, index) => {
              ...
              return (
                <div key={index} className="text-xl bg-white/30 rounded-xl p-4 mb-2 ">
                  ...
                  {Object.entries(movementData).map(([setKey, setInfo]) => {

                    const info = setInfo as {
                      reps?: number;
                      liftWeight_lbs?: number;
                      liftWeight_kg?: number;
                      locked?: boolean;
                    ...
                    const hasDropSets = info.dropset ?? false;
                    const dropSetList = info.dropsets ?? {};


                    const setNumber = setKey.replace("set", ""); // "set1" → "1"
                    return (
                      ...
                      <>
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setWorkoutSessionData((prev) => ({
                                      ...
                                      [movementName]: {
                                        ...prev[movementName],
                                        [setKey]: {
                                          ...prev[movementName]?.[setKey],
                                          liftWeight_lbs:
                                            preferredWeightUnit === "lbs" ? value : prev[movementName]?.[setKey]?.liftWeight_lbs,
                                          liftWeight_kg:
                                            preferredWeightUnit === "kg" ? value : prev[movementName]?.[setKey]?.liftWeight_kg,
                                        },
                                      },
                                    }));
                                  }}
                        ...
                        {hasDropSets && Object.entries(dropSetList).map(([dropKey, dropInfo]) => {
                          ...
                          return (
                            <div key={dropKey} className="mt-2 p-2 bg-white/10 rounded-lg">
                                  ...
                                  <div>
                                    <p className="text-sm text-white font-semibold mb-1">
                                      ...
                                      <input
                                        ...
                                        value={
                                          preferredWeightUnit === "lbs"
                                            ? dropLbs === undefined ? "" : dropLbs
                                            : dropKg === undefined ? "" : dropKg
                                        }
                                        onChange={(e) => {
                                          const value = parseFloat(e.target.value);
                                          setWorkoutSessionData((prev) => ({
                                            ...
                                            [movementName]: {
                                              ...
                                              [setKey]: {
                                                ...prev[movementName]?.[setKey],
                                                dropsets: {
                                                  ...prev[movementName]?.[setKey]?.dropsets,
                                                  [dropKey]: {
                                                    ...prev[movementName]?.[setKey]?.dropsets?.[dropKey],
                                                    liftWeight_lbs:
                                                      preferredWeightUnit === "lbs"
                                                        ? value
                                                        : prev[movementName]?.[setKey]?.dropsets?.[dropKey]?.liftWeight_lbs,
                                                    liftWeight_kg:
                                                      preferredWeightUnit === "kg"
                                                        ? value
                                                        : prev[movementName]?.[setKey]?.dropsets?.[dropKey]?.liftWeight_kg,
                                                  },
                                                },
                                              },
                                            },
                        ...
                        <button onClick={() => addDropSetToSet(movementName, setKey)}
                          className="bottom-0 mt-2 px-4 py-2 text-sm rounded bg-indigo-600 text-white hover:bg-indigo-500 w-fit">
                          + Add Drop Set
                        </button>
                      </>
                    );
                  ...
                  <button
                    ...
                    className=" mt-2 px-4 py-2 text-sm rounded bg-indigo-600 text-white hover:bg-indigo-500 w-fit"
                  >
                    + Add Set
                  </button>
          ...
          <div className="text-xl bg-white/30 rounded-xl p-4 mb-2 ">
            ...
            <div className="grid grid-cols-[4fr_1fr] gap-4">
                ...
                <div>
                  <p className="text-sm text-white font-semibold mb-1">
                    ...
                    <input
                      ...
                      step="0.1"
                      min="0"
                      value={preferredWeightUnit === "lbs" ? liftWeight_lbs.toString() : liftWeight_kg.toString()} onChange={handleWeightChange}
                      placeholder="Weight"
                      className="w-full p-2 mb-2 rounded bg-gray-800/70 text-white border-none focus:outline-none appearance-none"
                    />
                  </p>
              ...
              <div className="flex justify-center items-center">
                <button
                  ...
                  className={`flex rounded-2xl p-3 justify-center items-center ${locked === true
                    ? "bg-indigo-300/70 relative z-10 text-white font-bold px-4 py-2 rounded-xl overflow-hidden border border-indigo-400"
                    : "glowing-button"
                    }`}
                >
        ...
        <>
          ...
          <h2 className="text-sm font-bold text-white">
            Im sorry, It looks like you dont have an active session. Would you like to make one?
          </h2>
